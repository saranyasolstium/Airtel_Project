...
export async function listIncidentAlerts(params = {}, config = {}) {
  const res = await api.get("/api/incidents/alerts", { params, ...config });
  return res.data;
}

export async function resolveIncidentAlert(alertId) {
  const res = await api.put(`/api/incidents/alerts/${alertId}/resolve`);
  return res.data;
}

export async function getIncidentFilters(config = {}) {
  const res = await api.get("/api/incidents/alerts/filters", config);
  return res.data;
}

/** ✅ NEW: Stats API */
export async function getIncidentAlertStats(params = {}, config = {}) {
  // your backend url: /api/incidents/alerts/stats
  const res = await api.get("/api/incidents/alerts/stats", {
    params,
    ...config,
  });
  return res.data; // expects { summary: {total, active, resolved}, ... }
}
