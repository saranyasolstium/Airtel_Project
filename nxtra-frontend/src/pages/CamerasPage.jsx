...
/* ------------------------- Form Modal (EDIT camera_id supported) ------------------------- */
const CameraFormModal = ({ camera, onClose, onSave, mode = "create" }) => {
  ...
  // ✅ keep original camera_id (for update URL key)
  const originalIdRef = useRef(camera?.camera_id || "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      // ✅ send old_camera_id for edit
      const payload =
        mode === "edit"
          ? { ...formData, old_camera_id: originalIdRef.current }
          : { ...formData };
        ...
        <form onSubmit={handleSubmit} className="popupBody">
          <div className="formRow">
            ...
            <input
              ...
              disabled={saving} // ✅ allow editing camera_id also
            ...
            {mode === "edit" && (
              <div className="muted" style={{ marginTop: 6 }}>
                Original: <span className="mono">{originalIdRef.current}</span>
              </div>
            )}
          </div>
...
export default function CamerasPage() {
  ...
  // ✅ IMPORTANT: edit supports camera_id change (uses old_camera_id)
  const handleSaveCamera = async (payload) => {
    try {
      if (formMode === "create") {
        const saved = await CamerasAPI.create(payload);
        showToast(
          "success",
          "Camera Added",
        ...
        const oldId = payload.old_camera_id || payload.camera_id;
        const saved = await CamerasAPI.update(oldId, payload); // URL uses OLD id
        showToast(
          "success",
          "Camera Updated",
